...
public class Game extends Canvas implements Runnable {
	...
	public void init() {
		...
		raptor = new Raptor(level, "Raptor", Level.width/2 * 8, Level.height/2 * 8, 1);
		level.addEntity(raptor);
	...
	public synchronized void start() {
		...
		Object[] multiplayerChoices = { "Start a Server", "Join a Server" };
		Object defaultMultiplayerChoice = multiplayerChoices[0];
		// If the user decides to create a server
		if (JOptionPane.showOptionDialog(this, "Play with friends!", "Multiplayer Options", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE, null, multiplayerChoices, defaultMultiplayerChoice) == 0) {
		...
		if (socketServer != null) {
			Object[] levelChoices = { "Load a previous level", "Create a new level" };
			Object defaultLevelChoice = levelChoices[1];
			// If the user decides to create a server
			if (JOptionPane.showOptionDialog(this, "How do you want to play?", "Level Options",
					JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, levelChoices,
					defaultLevelChoice) == 0) {
				levelName = JOptionPane.showInputDialog("Please enter the name or title of the level: ");
				levelPath = levelName + ".png";
			} else {
				levelPath = null;
				if (Game.socketServer != null) {
					levelName = JOptionPane.showInputDialog("Give this level a name why don't you?");
				}
			}
		}
